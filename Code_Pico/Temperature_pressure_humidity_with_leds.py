...
while True:
 ...
 oled.fill(0)
 oled.text('Counter : '+ str(i),6,0)
 #oled.text('Counter:' + str(i),6,14)
 oled.text('Temp:' + tempC,6,28)
 oled.text('Pres:' + preshPa,6,42)
 oled.text('Humi:' + humRH,6,56)
 
 if float((str(round(temp,2)))) < 21 :
        led_blue.on()
 elif float((str(round(temp,2)))) > 25.0 :
        led_rouge.toggle()
 
 oled.show()
 i+=1
 utime.sleep(1) #Sleep for two seconds
